...
Solution.prototype.energy = def() :      # 能量函數:計算 ||AX-B||，也就是 ||Y-B||
  var X = self.v 
  var Y = A.mul(X) 
  return Y.sub(B).norm() 
...
Solution.prototype.toString = def() :    # 將解答轉為字串的函數，以供列印用。
  return "energy("+self.v.transpose().toString().replace("\n", "")+")="+self.energy().toFixed(3) 
