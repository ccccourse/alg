...
GA.run=def(size, maxGen) : # 遺傳演算法主程式
...
GA.dump = def() : # 印出一整代成員
  for (var i=0  i<GA.population.length  i++) :
    print(i, GA.population[i]) 
  


def random(a,b) : # 取得 a 到 b 之間的一個浮點亂數
  return a+Math.random()*(b-a) 


def randomInt(a,b) : # 取得 a 到 b 之間的一個整數亂數
  return Math.floor(random(a,b)) 


def randomChoose(array) : # 隨機取得 array 陣列的一個元素
...
KeyGA.randomChromosome=def() : # 隨機產生一個染色體 (一個 16 位元的 01 字串)
  var bits=[] 
...
KeyGA.calcFitness=def(c) : # 分數是和 key 一致的位元個數
  var fitness=0 
  for (var i=0  i<KeyGA.key.length  i++) :
...
KeyGA.mutate=def(chromosome) : # 突變運算
