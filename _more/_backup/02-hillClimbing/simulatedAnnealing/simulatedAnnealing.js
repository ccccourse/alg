var simulatedAnnealing = def() :       # 模擬退火法的物件模版 (類別)

simulatedAnnealing.prototype.P = def(e, enew, T) : # 模擬退火法的機率函數
  if (enew < e) 
    return 1 
  else
...
simulatedAnnealing.prototype.run = def(s, maxGens) : # 模擬退火法的主要函數
  ...
  for (var gens=0  gens<maxGens  gens++) :    # 迴圈，最多作 maxGens 這麼多代。
    ...
    T  = T * 0.999                            # 每次降低一些溫度
    if (self.P(e, enew, T) > Math.random()) : # 根據溫度與能量差擲骰子，若通過
      s = snew                                # 則移動到新的鄰居解
      print("%d T=%s %s", gens, T.toFixed(3), s.toString())  # 印出觀察
  ...
  print("solution: %s", sbest.toString())  # 印出最佳解
  return sbest                                   # 傳回最佳解
